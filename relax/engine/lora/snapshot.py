...
from relax.engine.lora.artifact import canonical
...
def fingerprint_model(directory: str | Path) -> str:
    ...
    return hashlib.sha256(canonical({path.name: _file_digest(path) for path in files})).hexdigest()
...
def snapshot_adapter(
    ...
    *,
    ...
    reserved_bytes = sum(item["size"] for item in files.values()) + len(canonical(planned))
    with _commit_lock(store):
