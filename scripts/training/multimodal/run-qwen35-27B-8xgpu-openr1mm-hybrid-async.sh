...
#
# Qwen3.5-27B 8xGPU hybrid-async training script (ray-job mode).
# Layout: 6 actor (TP=2, PP=3) + 2 rollout (TP=2).
# The Ray cluster is managed externally — do NOT kill ray or start a new cluster.
#
# Usage:
#   bash scripts/training/multimodal/run-qwen35-27B-8xgpu-openr1mm-hybrid-async.sh [hybrid-async|sync]
...
# Actor: 6 GPUs with TP=2, PP=3 (64 layers split 20/24/20 — embed on stage0, lm_head+MTP on stage2)
PERF_ARGS=(
...
WANDB_ARGS=(
   ...
   --tb-experiment-name qwen35-27b-GRPO-gpu8-${MODE}-${now}
)

# Rollout: 2 GPUs, TP=2 per engine
SGLANG_ARGS=(
...
if [ ${MODE} = "hybrid-async" ]; then
     ray job submit ${RAY_NO_WAIT:+--no-wait} --address="http://127.0.0.1:8265" \
        ...
        --resource '{"actor": [1, 6], "rollout": [1, 2]}'\
        --max-staleness 2 \
        ...
        "${PERF_ARGS[@]}" \
        "${SGLANG_ARGS[@]}" \
        "${MISC_ARGS[@]}"  2>&1 | tee log/qwen35-27b-GRPO-gpu8-hybrid-async-${now}.log
else
    ray job submit ${RAY_NO_WAIT:+--no-wait} --address="http://127.0.0.1:8265" \
         --runtime-env-json="${RUNTIME_ENV_JSON}" \
         -- python3 -m relax.entrypoints.train \
         ...
         "${PERF_ARGS[@]}" \
         "${SGLANG_ARGS[@]}" \
         "${MISC_ARGS[@]}"  2>&1 | tee log/qwen35-27b-GRPO-gpu8-fully-sync-${now}.log
fi
