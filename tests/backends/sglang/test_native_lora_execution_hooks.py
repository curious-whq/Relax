...
@pytest.mark.parametrize("entry", ["discard", "abort"])
async def test_native_cancel_entry_retains_owner_until_completion(native, entry):
...
@pytest.mark.parametrize("kind", ["warmup", "probe"])
...
async def test_native_control_generation_requires_success(native, result, kind):
    tokenizer, _, protocol = native
    manager = tokenizer.TokenizerManager.__new__(tokenizer.TokenizerManager)
    identity = protocol.LoRAExecutionIdentity(
        "cohort", "boot", "uid" if kind == "warmup" else None, "prepare", "session", "rid", kind=kind
    )
    ref = SimpleNamespace(lora_id="uid", lora_name="version-name", pinned=True) if kind == "warmup" else None
    sent = []
    ...
    if valid:
        ...
        with pytest.raises(RuntimeError, match="INTERNAL_GENERATION_FAILED"):
            await manager._generate_lora_control_token(identity, ref)
    obj = sent[0]
    assert obj.lora_path == (ref.lora_name if ref else None) and obj._lora_execution_ref is ref
