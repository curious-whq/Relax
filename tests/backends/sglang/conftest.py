...
async def prepare_version(control, identity, digest, ref, warmup) -> str:
    control.require_resident()
    record = control._version(identity, digest, ref)
    if record.fenced:
        raise ValueError("VERSION_FENCED")
    if record.state == "READY":
        return record.state
    state = await asyncio.shield(control._start_work(record, warmup))
    if record.error:
