...
from .processes import engine_options
...
def _arguments(config: dict, output: Path) -> argparse.Namespace:
    ...
    values = {
        ...
        **{f"sglang_{name}": value for name, value in engine_options(config).items()},
...
@contextmanager
def start_cluster(config: dict) -> Iterator[SimpleNamespace]:
