...
# Each task declares only the independent reference weights it exercises.
TASKS = {
    "publication": ("A", "B"),
    "sessions": ("A", "B"),
    "capacity": ("B",),
...
async def check_managed_profiles(ctx) -> None:
    ...
    for engine_id in state["serving_engines"]:
        ...
        profiles[engine_id] = inference_profile(response.json(), deterministic=ctx.config.get("deterministic", False))
...
async def execute(config: dict, cluster, report: dict) -> None:
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(10, read=120), trust_env=False) as client:
        try:
            if config["task"] in ("publication", "sessions"):
                ...
                if config["task"] == "sessions":
                    train_runtime = await RuntimeDomain.connect(
                        Namespace(rollout_global_dataset=True), "train", concurrency=1
                    )
            ctx = SimpleNamespace(
                config=config,
                report=report,
...
def worker(config: dict) -> None:
    ...
    try:
        ...
        with start_cluster(config) as cluster, ExitStack() as stack:
            ...
            for index, name in enumerate(TASKS[config["task"]]):
                # Keep B on the same selected GPU as the A/B reference profile.
                gpu = config["gpus"][index if config["task"] == "performance" else {"A": 0, "B": 1}[name]]
                label = f"ordinary-{index}" if config["task"] == "performance" else f"baseline-{name}"
                ...
                if config["task"] == "performance":
                    reference["hardware"] = (
                        f"same selected GPU {gpu['uuid']}, TP=1, BF16, Triton LoRA, "
                        f"memory fraction {config.get('mem_fraction', 0.2)}, graph enabled"
                    )
                    config["performance_baselines"].append(reference)
                else:
                    config["baselines"][name] = reference
