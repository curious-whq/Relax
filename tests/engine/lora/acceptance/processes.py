...
def owned_process(command: list[str], env: dict, log: Path, records: list):
    record = {"command": command, "log": str(log), "cleanup": "PENDING"}
...
def engine_options(config: dict) -> dict:
    """Keep managed and independent reference engines on the same profile."""
    options = {
        "tp_size": 1,
        "dtype": "bfloat16",
        "lora_backend": "triton",
        "mem_fraction_static": float(config.get("mem_fraction", 0.2)),
        ...
        "enable_metrics": bool(config.get("native_metrics", False)),
    }
    if config.get("deterministic", False):
        # Deterministic FlashInfer disables radix caching in v0.5.17.
        options.update(enable_deterministic_inference=True, attention_backend="triton")
    return options
...
def baseline(
    ...
    for name, value in engine_options(config).items():
        flag = "--" + name.replace("_", "-")
    ...
    command += [
        "--enable-lora",
        "--max-lora-rank",
        str(config.get("lora_rank", 8)),
        "--lora-target-modules",
        *config.get("lora_target_modules", ["q_proj", "v_proj"]),
        "--max-loras-per-batch",
        "3",
        "--max-loaded-loras",
        "3",
    ...
    with owned_process(command, env, log, records) as process:
        with httpx.Client(base_url=url, timeout=5, trust_env=False) as client:
            ...
            profile = inference_profile(response.json(), deterministic=config.get("deterministic", False))
        yield {"url": url, "lora_path": version, "cache_enabled": True, "effective_profile": profile}


def inference_profile(info: dict, *, deterministic: bool) -> dict:
